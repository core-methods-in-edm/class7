...
```{r}
student_activity_wide<- spread(student_data, variable, measure)
student_t4<-dplyr::filter(student_activity_wide, table == 4)
...
```{r}
student_data <- dplyr::mutate(student_activity_wide, week = dplyr::ntile(date, 3))
```
...
a <- as.numeric(unlist(instructor_week[,2]))
b <- as.numeric(unlist(student_week[,2]))
cor.test(a,b)
```
