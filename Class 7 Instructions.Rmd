...
```{r}
data_wide_2 <- read.table("student_activity_wide.csv", sep = ",", header = TRUE)
> student_data_1 <- spread(data_wide_2,variable, measure)
...
```{r}
student_data_3 <- dplyr::filter(student_data_1, table == 4)
```
...
```{r}
student_data_1 <- dplyr::mutate(student_data_1, week = dplyr::ntile(date, 3))
```
...
```{r}
student_data_4<- dplyr::mutate(student_data_1 %>% dplyr::group_by(week) %>% dplyr::summarise(mean(motivation)))
instructor_week<- dplyr::mutate(instructor_data_1 %>% dplyr::group_by(week) %>% dplyr::summarise(mean(motivation)))
```
...
```{r}
plot(instructor_week,student_data_4)
plot(student_data_4,instructor_week)
x_motivation<-c(1.666667,1.851852,1.851852)
y_active_time<-c(6913.25,6240.286,5956.143)
cor.test(x_motivation,y_active_time)
```
