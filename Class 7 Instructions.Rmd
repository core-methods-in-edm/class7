...
```{r}
student_wide <- spread(student_activity, variable, measure)
```
...
```{r}
student_wide <- dplyr::mutate(student_wide, week = dplyr::ntile(date, 3))
```
...
```{r}
student_week <- student_wide %>% dplyr::group_by(week) %>% dplyr::summarise(mean(motivation))
...
cor.test(merge$`mean(m_active_time)`, merge$`mean(motivation)`)
 ### Or the opposite, for the same correlation results but different plot view. ###
