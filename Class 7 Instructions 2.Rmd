...
```{r}
data_wide <- read.table("C:/Users/Ben/Documents/class7/instructor_activity_wide.csv", sep = ",", header = TRUE)
...
```{r}
data_wide <- read.table("C:/Users/Ben/Documents/class7/student_activity.csv", sep = ",", header = TRUE)
View(data_wide)
...
```{r}
student_data_2 <- dplyr::filter(student_data_2, date == 20160204)
```
...
```{r}
student_data.sub <- subset(student_data_2, student_data_2$measure == 4)
```
...
```{r}
instructor_sleep<- data.frame(instructor_data$total_sleep)
...
```{r}
student_data_2<- dplyr::mutate(student_data_2, week = dplyr::ntile(date, 3))
...
View(student_data_2)
```{r}
student_data_2 %>%dplyr::summarise(mean(motivation))
student_data.sub %>% dplyr::summarise(mean(student_data.sub$measure))

#That isn't super interesting, so let's break it down by week:

student_data.sub %>% dplyr::group_by(date) %>% dplyr::summarise(mean(student_data.sub$measure))
```
...
```{r}
student_data_2 %>% dplyr::summarise(mean(student_data_2$week))

instructor_data %>% dplyr::summarise(mean(instructor_data$m_active_time))
```
...
```{r}
merge <- dplyr::full_join(instructor_data$week, m_active_time, "week")
```
...
```{r}
plot(instructor_data$m_active_time, student_data_2$week)

cor.test(instructor_data$m_active_time, student_data_2$week)
